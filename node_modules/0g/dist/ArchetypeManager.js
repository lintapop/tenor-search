...
var ArchetypeManager = /** @class */ (function (_super) {
    ...
    ArchetypeManager.prototype.addComponent = function (entityId, instance) {
        logger_1.logger.debug("Adding " + instance.type + " to entity " + entityId);
        var oldArchetypeId = this.entityLookup[entityId];
        if (oldArchetypeId === undefined) {
            throw new Error("Tried to add component " + instance.type + " to " + entityId + ", but it was not found in the archetype registry");
        }
        ...
        var newArchetypeId = (this.entityLookup[entityId] = this.flipBit(oldArchetypeId, instance.type));
    ...
    ArchetypeManager.prototype.removeComponent = function (entityId, componentType) {
        logger_1.logger.debug("Removing " + componentType + " from entity " + entityId);
        var oldArchetypeId = this.entityLookup[entityId];
        if (oldArchetypeId === undefined) {
            throw new Error("Tried to remove component " + componentType + " from " + entityId + ", but it was not found in the archetype registry");
        }
        var oldArchetype = this.getOrCreate(oldArchetypeId);
        var entity = oldArchetype.removeEntity(entityId);
